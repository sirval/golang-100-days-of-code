...
func main() {
	...
	for {
		...
		if userTickets > conferenceTicket {
			fmt.Printf("We have only %v available. You can't book %v ticket\n", conferenceTicket, userTickets)
			break
		} else if userTickets > remainingTicket {
			fmt.Printf("Sorry, we have only %v available. You can't book %v ticket\n", remainingTicket, userTickets)
			break
		} else if remainingTicket == 0 {
			fmt.Println("Conference ticket sold out. Come back next year")
			break
		} else {

			// remainingTicket = conferenceTicket - userTickets
			// if remainingTicket > userTickets {
			// 	continue
			// }
